...
// 3. If upload fails remove the the document added previously and show message 
    ...
    this.firestorePostRef.add({
        ...
        .then(async()=>{
            ...
            await postFileRef.getDownloadURL().then((url)=> {
            
                this.setState({mediaFile: url});
                this.firestorePostRef.doc(post.id).set({ 
                    postId: post.id,
...
uploadPost = async (uri, fileName)=>{
    ...
    const ref = firebase.storage().ref().child("postImages/"+ fileName);
    return ref.put(blob);
}}
                          ...
                          <React.Fragment>
                            ...
                            <ScrollView style={{maxHeight: 100}}>
                              {locationResults.map((el, i) => (
                                <TouchableOpacity key={i} style={styles.searchItem}>
